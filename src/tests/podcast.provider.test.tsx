...
import { PodcastProvider, usePodcast } from '../providers/Podcast.provider';
...
describe('PodcastProvider', () => {
  ...
  test('searchPodcast returns matching podcasts', async () => {
    const { result, unmount } = renderHook(() => usePodcast(), {
      wrapper: PodcastProvider,
    });
    expect(result.current.podcasts).toEqual([]);
    await waitFor(() => {
      result.current.searchPodcast('');
    });
    expect(result.current.podcasts).toEqual(podcastData);
    unmount();
  });

  test('searchPodcast with  name returns matching podcasts', async () => {
    const { result, unmount } = renderHook(() => usePodcast(), {
      wrapper: PodcastProvider,
    });
    await waitFor(() => {
      result.current.searchPodcast(podcastData[1].name);
    });
    expect(result.current.podcasts).toEqual([podcastData[1]] as Podcast[]);
    unmount();
  });

  test('findPodcast returns matching podcast', async () => {
    const { result, unmount } = renderHook(() => usePodcast(), {
      wrapper: PodcastProvider,
    });
    await waitFor(() => {
    ...
    await waitFor(async () => {
      await result.current.findPodcast('1535809341');
    });

    expect(result.current.currentPodcast.podcast).toEqual(podcastData[0]);
    expect(result.current.currentPodcast.total).toEqual(3);
    expect(result.current.currentPodcast.episodes).toEqual(episodeData);
    unmount();
  });

  test('findEpisode returns matching episode', async () => {
    const { result, unmount } = renderHook(() => usePodcast(), {
      wrapper: PodcastProvider,
    });
    await waitFor(() => {
    ...
    await waitFor(() => {
      result.current.findPodcast('1535809341');
    });
    await waitFor(() => {
      result.current.findEpisode('1000608319666');
    });
    expect(result.current.episode).toEqual(episodeData[2]);
    unmount();
  });
});
